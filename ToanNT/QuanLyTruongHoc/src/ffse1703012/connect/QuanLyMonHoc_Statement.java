...
public class QuanLyMonHoc_Statement {
	final Connection conn= (Connection) (new SinhVienDAO()).getConnect("localhost", "ffse1703012_java", "toan123", "123");
	ArrayList<QuanLyMonHoc> arrMonHoc = new ArrayList<>();

	public ArrayList<QuanLyMonHoc> sellectMonHoc() {
		...
		try {
			...
			while (result.next()) {
				...
				arrMonHoc.add(new QuanLyMonHoc(maMon, tenMon, tinChi, thoiLuong));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void insertMonHoc(String maMon, String tenMon, String soTinChi, String thoiLuong) {
		try {
			String sql = "insert into Mon_hoc values('" + 0 + "','" + maMon + "', '" + tenMon + "', '" + soTinChi
					+ "','" + thoiLuong + " giờ" + "')";
			Statement statement = conn.createStatement();
	...
	public void updateMonHoc(String maMon, String tenMon, String soTinChi, String thoiLuong) {
		try {
			String sql = "update Mon_hoc set Tenmon='" + tenMon + "', SoTinChi='" + soTinChi + "', Thoiluong='"
					+ thoiLuong + " giờ" + "' where Mamon='" + maMon + "'";
			Statement statement = conn.createStatement();
