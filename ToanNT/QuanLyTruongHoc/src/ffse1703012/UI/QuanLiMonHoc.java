...
public class QuanLiMonHoc extends JFrame{
	/**
	 * 
	 */
	...
	private ArrayList<QuanLyMonHoc> arrMonHoc = new ArrayList<>();
	private QuanLyMonHoc_Statement quanLiMonHocStatement = new QuanLyMonHoc_Statement();
	public QuanLiMonHoc() {
		addControl();
		addEvent();
	}
	...
	private void addControl() {
		...
		tbSinhVien.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				for(QuanLyMonHoc x : arrMonHoc) {
					textThoiLuong.setForeground(new Color(0, 0, 0));
					if(value.equals(x.getMaMH())) {
						textMaMon.setText(x.getMaMH());
						textTenMon.setText(x.getTenMH());
						textSoTinChi.setText(x.getSoTinChi());
						int endSubtring = x.getThoiGianHoc().length() - 4;
						textThoiLuong.setText(x.getThoiGianHoc().substring(0, endSubtring));
	...
	public void in() {
		for(QuanLyMonHoc x : arrMonHoc) {
			String row[] = {x.getMaMH(), x.getTenMH(), x.getSoTinChi(), x.getThoiGianHoc()};
			dm.addRow(row);
		}
	}
