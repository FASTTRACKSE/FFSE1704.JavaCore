package fasttrack.quanlycanboluufile.main;

import fasttrack.quanlycanboluufile.entity.CanBo;
import fasttrack.quanlycanboluufile.entity.GiangVien;
import fasttrack.quanlycanboluufile.entity.NhanVien;
import fasttrack.quanlycanboluufile.util.myException;
import fasttrack.quanlycanboluufile.util.SerializeFileFactory;
	...
	public static void sapXepTheoLuong() {
		...
		for (CanBo x : dsCanBo) {
			...
			if (x instanceof NhanVien) {
				System.out.printf("\n" + "%-10s %-10s %-15s %-15s %-15s %-15s %-15s \n", "Họ tên ", "PhongBan", "Chức Vụ",
